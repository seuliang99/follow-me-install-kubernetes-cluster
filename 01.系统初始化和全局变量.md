...
一般 DNS 不支持解析主机名称，需要修改每台机器的 `/etc/hosts` 文件，添加主机名和 IP 的对应关系：
...
``` bash
sudo modprobe br_netfilter  #modprobe用于向内核中加载模块或者从内核中移除模块。
```
...
在master上，后续的部署步骤将使用 [environment.sh](manifests/environment.sh) 文件中定义的全局环境变量，请根据**自己的机器、网络情况**修改：

然后，把全局变量定义脚本拷贝到**所有**节点的 `/opt/k8s/bin` 目录：

``` bash
cd /opt/k8s/bin/
wget https://raw.githubusercontent.com/seuliang99/follow-me-install-kubernetes-cluster/master/manifests/environment.sh
``` bash
